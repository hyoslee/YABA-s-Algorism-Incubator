...
public class HelloYaba {
	...
	public String[] eArr= {"027830",  "002810",  "123700",  "263750",  "005880",  "089010",  "004020",  "017000",  "112610",  "082920",  "096770",  "051910",  "035760",  "041910",  "006620",  "001750",  "097780",  "033500",  "012450",  "005830"};
	...
	public Integer idxLong = 21;
	public Integer bIndex = 20;
	public Integer adj = 1;
	...
	public void getInfo(String itm) {
		...
		try {
			...
			if(((JSONObject)jItm.get(6)).length() > 1 && ((JSONObject)jItm.get(7)).length() > 1) {
				System.out.print(((JSONObject)jItm.get(6)).get("content") + "\t");
				System.out.println(((JSONObject)jItm.get(7)).get("content"));
...
//			if(Math.round(amtMax / Double.parseDouble(jData3.split("\\|")[5]) * 100)/100.0 < 2) {
			if(1>0) {	
				...
				els.add(new ELS(itm, Math.round((b2-b1)/b1*10000)/100.0, Math.round(amtMax / Double.parseDouble(jData3.split("\\|")[5]) * 100)/100.0));
